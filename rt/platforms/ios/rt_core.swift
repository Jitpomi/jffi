...
fileprivate extension RustBuffer {
    ...
    static func from(_ ptr: UnsafeBufferPointer<UInt8>) -> RustBuffer {
        try! rustCall { ffi_rt_core_rustbuffer_from_bytes(ForeignBytes(bufferPointer: ptr), $0) }
    }

    // Frees the buffer in place.
    // The buffer must not be used after this is called.
    func deallocate() {
        try! rustCall { ffi_rt_core_rustbuffer_free(self, $0) }
    }
}
...
open class Core: CoreProtocol, @unchecked Sendable {
    ...
    public func uniffiCloneHandle() -> UInt64 {
        return try! rustCall { uniffi_rt_core_fn_clone_core(self.handle, $0) }
    }
public convenience init() {
    let handle =
        try! rustCall() {
    uniffi_rt_core_fn_constructor_core_new($0
    )
}
    ...
    deinit {
        ...
        try! rustCall { uniffi_rt_core_fn_free_core(handle, $0) }
    }
...
open func greeting() -> String  {
    ...
    uniffi_rt_core_fn_method_core_greeting(
...
private let initializationResult: InitializationResult = {
    ...
    let scaffolding_contract_version = ffi_rt_core_uniffi_contract_version()
    if bindings_contract_version != scaffolding_contract_version {
        return InitializationResult.contractVersionMismatch
    }
    if (uniffi_rt_core_checksum_method_core_greeting() != 17636) {
        return InitializationResult.apiChecksumMismatch
    }
    if (uniffi_rt_core_checksum_constructor_core_new() != 26045) {
        return InitializationResult.apiChecksumMismatch
    }
...
public func uniffiEnsureRtCoreInitialized() {
