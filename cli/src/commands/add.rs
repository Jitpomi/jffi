...
pub fn add_platform(platform: &str) -> Result<()> {
    println!(
        "{}",
        format!("➕ Adding {} platform...", platform)
            .bright_green()
            .bold()
    );
    println!();
    ...
    if platform == "web" {
        ...
        let all_platforms: Vec<&str> = config
            .platforms
            .enabled
            .iter()
            .map(|s| s.as_str())
...
"#;
    ...
    println!("Next steps:");
    println!(
        "  {} - Compile the core and generate bindings",
        format!("jffi build --platform {}", platform).bright_cyan()
    );
    println!(
        "  {} - Build and launch on simulator/device",
        format!("jffi run --platform {}", platform).bright_cyan()
    );
    println!(
        "  {} - Watch mode (auto-rebuild on changes)",
        format!("jffi dev --platform {}", platform).bright_cyan()
    );
