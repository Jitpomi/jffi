...
fn build_android(release: bool) -> Result<()> {
    ...
    // Check if ndk-context initialization is needed
    let needs_ndk_context = check_ndk_context_needed();
    if needs_ndk_context {
        println!("  {} Detected ndk-context dependency - will generate JNI bridge", "ℹ".bright_blue());
    }
    ...
    // Generate ndk-context JNI bridge if needed
    if needs_ndk_context {
...
fn generate_android_ndk_bridge() -> Result<()> {
    println!("  {} Generating ndk-context JNI bridge...", "→".bright_blue());
...
pub unsafe extern "C" fn Java_{jni_package}_JffiAndroidInit_initNdkContext(
    ...
    // Initialize ndk-context with the JVM and context
    ndk_context::initialize_android_context(&mut env, global_context.as_obj());
...
object JffiAndroidInit {{
    init {{
        System.loadLibrary("{lib_name}_core")
    }}
