...
fn validate_project_structure() -> Result<()> {
    ...
    // Builds must never rewrite user source. Report the problem and let the
    // developer make the API decision explicitly.
    ensure_public_modules()?;
...
fn ensure_public_modules() -> Result<()> {
    ...
    let mut private_modules = Vec::new();
    ...
    for line in content.lines() {
        ...
        {
            private_modules.push(trimmed.to_string());
        }
    }

    if !private_modules.is_empty() {
        anyhow::bail!(
            "UniFFI cannot export items from private modules in core/src/lib.rs: {}. \
             Change each intended API module to `pub mod ...;`; JFFI will not rewrite source during a build.",
...
pub fn build_platform_with_options(
    platform: &str,
    release: bool,
    device: bool,
    deploy: bool,
...
fn build_all_platforms(release: bool) -> Result<()> {
    ...
    for platform in &config.platforms.enabled {
        ...
        if let Err(e) = build_platform(platform, release) {
            ...
            failures.push(format!("{}: {:#}", platform, e));
        } else {
        ...
        if universal_dir.exists() {
            if let Err(e) = std::fs::remove_dir_all(universal_dir) {
                if attempt < 2 {
                    ...
                    eprintln!(
                        "  {} Warning: Could not clean ios-simulator-universal directory: {}",
                        "⚠".yellow(),
                        e
                    );
                }
        ...
        if let (Some(sim), Some(dev)) = (sim_dir, device_dir) {
            ...
            match (
                ...
                _ => {
                    ...
                    println!(
                        "  {} XCFramework update failed, recreating...",
                        "⚠".yellow()
                    );
            ...
            println!(
                "  {} XCFramework structure incomplete, recreating...",
                "⚠".yellow()
            );
            let _ = std::fs::remove_dir_all(&xcframework_path);
        }
...
"#;
    ...
    std::fs::write(&config_path, config_content).context("Failed to write .cargo/config.toml")?;
...
fn build_android(release: bool) -> Result<()> {
    ...
    if needs_ndk_context && no_bridge {
        println!(
            "  {} Skipping ndk-context JNI bridge (--no-android-bridge)",
            "ℹ".bright_blue()
        );
    } else if needs_ndk_context {
        println!(
            "  {} Detected ndk-context dependency - generating JNI bridge...",
            "ℹ".bright_blue()
        );
    ...
    if let Some(abis) = &config.platforms.android.abis {
        ...
        if architectures.is_empty() {
            anyhow::bail!(
                "No valid Android ABIs configured in jffi.toml. Enabled ABIs: {:?}",
                abis
            );
        }
    }
    ...
    if let Some(ndk_dir) = find_ndk_dir() {
        for (target, abi) in &architectures {
            if let Some(libcxx_path) = find_libcxx_shared(&ndk_dir, target) {
                ...
                if let Err(e) = std::fs::create_dir_all(&dest_dir) {
                    println!(
                        "  {} Warning: Failed to create jniLibs directory: {}",
                        "⚠".yellow(),
                        e
                    );
                } else if let Err(e) = std::fs::copy(&libcxx_path, &dest_path) {
                    println!(
                        "  {} Warning: Failed to copy libc++_shared.so for {}: {}",
                        "⚠".yellow(),
                        abi,
                        e
                    );
        ...
        println!(
            "  {} Warning: Could not find Android NDK directory to bundle libc++_shared.so",
            "⚠".yellow()
        );
    }
...
fn generate_android_ndk_bridge() -> Result<()> {
    ...
    let android_rs = format!(
        r#"//! Android-specific JNI bridge for ndk-context initialization
//!
...
"#
    ...
    let mut lib_rs =
        std::fs::read_to_string(lib_rs_path).context("Failed to read core/src/lib.rs")?;
    ...
    if !lib_rs.contains("mod android;") {
        ...
        if let Some(pos) = lib_rs.find("uniffi::include_scaffolding!") {
            ...
            std::fs::write(lib_rs_path, lib_rs).context("Failed to update core/src/lib.rs")?;
        }
    }
    ...
    let mut cargo_toml =
        std::fs::read_to_string(cargo_toml_path).context("Failed to read core/Cargo.toml")?;
    ...
    if !cargo_toml.contains("ndk-context") {
        ...
        std::fs::write(cargo_toml_path, cargo_toml).context("Failed to update core/Cargo.toml")?;
    }

    // 4. Generate Kotlin JffiAndroidInit.kt
    let package_path = package_name.replace(".", "/");
    let kotlin_dir = format!("platforms/android/app/src/main/java/{}", package_path);
    std::fs::create_dir_all(&kotlin_dir).context("Failed to create Kotlin package directory")?;

    let kotlin_init = format!(
...
"#
    ...
    std::fs::write(&kotlin_path, kotlin_init).context("Failed to write JffiAndroidInit.kt")?;

    println!("  {} Generated JNI bridge and Kotlin helper", "✓".green());
    println!(
        "  {} Add to MainActivity: JffiAndroidInit.initNdkContext(applicationContext)",
        "ℹ".bright_yellow()
    );
...
fn build_macos_xcframework(release: bool) -> Result<()> {
    ...
    if verbose {
        ...
        for (target, target_name) in &targets {
            ...
            if !rustflags_val.contains("-install_name") {
                ...
                rustflags_val.push_str(&format!(
                    "-C link-arg=-Wl,-install_name,@rpath/lib{}.dylib",
                    lib_name
                ));
            }
    ...
    let arm64_lib = format!(
        "target/aarch64-apple-darwin/{}/lib{}.dylib",
        profile, lib_name
    );
    let x86_lib = format!(
        "target/x86_64-apple-darwin/{}/lib{}.dylib",
        profile, lib_name
    );
    ...
    let lipo_status = Command::new("lipo")
        .args(["-create", &arm64_lib, &x86_lib, "-output", &universal_lib])
        .status()
    ...
    if std::path::Path::new(&xcframework_path).exists() {
        ...
        if let Some(dir) = macos_dir {
            ...
            println!(
                "  {} XCFramework structure not found, recreating...",
                "⚠".yellow()
            );
            let _ = std::fs::remove_dir_all(&xcframework_path);
        }
...
fn build_windows_archs(
    ...
    for (i, arch) in archs.iter().enumerate() {
        match check_windows_toolchain(arch) {
            ...
            Ok(false) => {
                ...
                let msg = match *arch {
                    "aarch64" => "Install LLVM/Clang: winget install LLVM.LLVM",
                    "x86_64" | "i686" => "Install Visual Studio Build Tools with C++ workload",
    ...
    if !skipped.is_empty() {
        println!(
            "  {} Skipping unsupported architectures:",
            "⚠".bright_yellow()
        );
        for s in &skipped {
            println!("     • {}", s);
    ...
    let lib_name = std::env::current_dir()?
        ...
        .replace("-", "_");
    let lib_path = format!(
        "target/x86_64-pc-windows-msvc/{}/{}_core.dll",
        profile, lib_name
    );

    println!(
        "  {} Generating C# bindings with uniffi-bindgen-cs...",
        "→".bright_blue()
    );
    ...
    if let Ok(content) = std::fs::read_to_string(&cs_file_path) {
        ...
        if patched {
            ...
            println!(
                "  {} Auto-fixed uniffi-bindgen-cs interface prefix bug",
                "ℹ".bright_blue()
            );
        }
    }
    ...
    println!(
        "  {} Copying DLL to project directory...",
        "→".bright_blue()
    );
    ...
    let build_cmd: &str = if dotnet_cmd.is_some() {
        "dotnet"
    } else {
        "msbuild"
    };
...
pub(crate) fn ensure_uniffi_bindgen_cs() -> Result<()> {
    println!("  {} Checking uniffi-bindgen-cs...", "→".bright_blue());

    let check = Command::new("uniffi-bindgen-cs").arg("--version").output();

    if check.is_err() || !check.unwrap().status.success() {
        if !crate::setup::installation_allowed() {
            anyhow::bail!("uniffi-bindgen-cs is required. Run `jffi setup --platform windows`");
        }
        ...
        let tag = std::env::var("JFFI_UNIFFI_BINDGEN_CS_TAG")
            .unwrap_or_else(|_| "v0.10.0+v0.29.4".to_string());
        let branch = std::env::var("JFFI_UNIFFI_BINDGEN_CS_BRANCH").ok();

        println!(
            "    Installing uniffi-bindgen-cs ({} @ {})...",
            repo,
            branch.as_deref().unwrap_or(&tag)
        );
        ...
        let mut cmd = Command::new("cargo");
        cmd.env("CARGO_NET_GIT_FETCH_WITH_CLI", "true").args([
            "install",
            "uniffi-bindgen-cs",
            "--git",
            &repo,
        ...
        println!(
            "    {} uniffi-bindgen-cs installed successfully",
            "✓".green()
        );
    } else {
...
fn build_linux(release: bool) -> Result<()> {
    ...
    if python_check.is_err() || !python_check.unwrap().status.success() {
        anyhow::bail!(
            "Python GTK4 dependencies are not available. Run `jffi setup --platform linux`"
        );
    }
...
fn ensure_wasm_target() -> Result<()> {
    println!(
        "  {} Checking wasm32-unknown-unknown target...",
        "→".bright_blue()
    );
    ...
    if !installed.contains("wasm32-unknown-unknown") {
        anyhow::bail!("wasm32-unknown-unknown is not installed. Run `jffi setup --platform web`");
    }

    Ok(())
}

pub(crate) fn ensure_wasm_bindgen_cli() -> Result<()> {
    ...
    let required_version =
        required_version.context("Could not find wasm-bindgen version in Cargo.lock")?;

    // Check installed version
    let check = Command::new("wasm-bindgen").arg("--version").output();
    ...
    if needs_install {
        if !crate::setup::installation_allowed() {
            anyhow::bail!(
                "wasm-bindgen-cli {} is required. Run `jffi setup --platform web`",
                required_version
            );
        }
        println!(
            "    Installing wasm-bindgen-cli {} (this may take a few minutes)...",
            required_version
        );
        let status = Command::new("cargo")
            .args([
        ...
        println!(
            "  {} wasm-bindgen-cli {} already installed",
            "✓".green(),
            required_version
        );
    }
...
fn patch_swift_file(path: &Path) -> Result<()> {
    ...
    if patched != content {
        ...
        println!(
            "    {} Patched Swift 6 concurrency in {}",
            "✓".green(),
...
pub fn sync_configs_to_platforms(config: &crate::config::Config) -> Result<()> {
    ...
    if used_ci_run_number {
        println!(
            "    {} No build_number configured. Using CI run number: {}",
            "★".bright_magenta(),
            version_code
        );
    }
    ...
    if gradle_path.exists() {
        ...
        let old_package = content
            ...
            .and_then(|line| {
                ...
                if parts.len() == 2 {
                    ...
                    let trimmed_val = val.trim_matches(|c| {
                        c == '"' || c == '\'' || c == ' ' || c == ';' || c == ','
                    });
        ...
        let bundle_compile_sdk = config
            .bundle
            .as_ref()
            .and_then(|bundle| bundle.android.as_ref())
            .map(|android| android.compile_sdk);
        let target_sdk =
            resolve_android_target_sdk(config.platforms.android.target_sdk, bundle_compile_sdk);
        ...
        if new_content != content {
            ...
            println!(
                "    {} Synced Android gradle configurations (package: {})",
                "✓".green(),
                package
            );
        }
    ...
    for platform_str in platforms {
                            ...
                            .map(|line| {
                                ...
                                if trimmed.starts_with("PRODUCT_BUNDLE_IDENTIFIER = ") {
                                    ...
                                    if parts.len() == 2 {
                                        ...
                                        let existing_id =
                                            parts[1].trim().trim_end_matches(';').trim_matches('"');
                                        let final_id = if let Some(ref base) = base_id {
                                            if existing_id.starts_with(base)
                                                && existing_id.len() > base.len()
                                            {
                                        ...
                                        format!(
                                            "{:width$}PRODUCT_BUNDLE_IDENTIFIER = {};",
                                            "",
                                            final_id,
                                            width = indent
                                        )
        ...
        if platform_str == "ios" {
            if let Some(ref groups) = config.platforms.ios.app_groups {
                sync_entitlements(Path::new("platforms/ios"), groups)?;
            }
        }
        if platform_str == "macos" {
            if let Some(ref groups) = config.platforms.macos.app_groups {
                sync_entitlements(Path::new("platforms/macos"), groups)?;
            }
        }
    }
    ...
    if index_path.exists() {
        ...
        if let (Some(start), Some(end)) =
            (new_content.find("<title>"), new_content.find("</title>"))
        {
        ...
        if new_content != content {
            ...
            println!(
                "    {} Synced web SEO & meta tags (title: {})",
                "✓".green(),
                title
            );
        }
    ...
    if windows_path.exists() {
        ...
        if new_content != content {
            ...
            println!(
                "    {} Synced Windows Package.appxmanifest identity & version (id: {})",
                "✓".green(),
                windows_app_id
            );
        }
    ...
    if linux_app_py.exists() {
        ...
        if new_content != content {
            ...
            println!(
                "    {} Synced Linux application ID (id: {})",
                "✓".green(),
                app_id
            );
        }
...
fn get_linux_app_id(config: &crate::config::Config) -> String {
    ...
    if !config.platforms.android.package.is_empty()
        && config.platforms.android.package != "com.example.app"
    {
        return config.platforms.android.package.clone();
    }
    if !config.platforms.ios.bundle_id.is_empty()
        && config.platforms.ios.bundle_id != "com.example.app"
    {
...
fn rename_android_package(old_pkg: &str, new_pkg: &str) -> Result<()> {
    ...
    println!(
        "    {} Android package name change detected: {} → {}",
        "→".bright_blue(),
        old_pkg,
        new_pkg
    );
        ...
        if let Some(key_idx) = content.find("<key>com.apple.security.application-groups</key>") {
            if let Some(array_start) = content[key_idx..].find("<array>") {
                if let Some(array_end) = content[key_idx + array_start..].find("</array>") {
                    let line_start = content[..key_idx].rfind('\n').map_or(0, |idx| idx + 1);
                    let array_end_pos = key_idx + array_start + array_end + "</array>".len();
                    let suffix_start = content[array_end_pos..]
                        .find(|ch: char| !ch.is_whitespace())
                        .map_or(content.len(), |idx| array_end_pos + idx);
                    let mut res = content[..line_start].to_string();
                    res.push_str(&groups_xml);
                    if suffix_start < content.len()
                        && !content[suffix_start..].starts_with("</dict>")
                    {
                        res.push('\t');
                    }
                    res.push_str(&content[suffix_start..]);
                    res
                } else {
                    content.clone()
                }
...
mod config_sync_tests {
    ...
    fn explicit_build_number_wins_over_ci_run_number() {
        assert_eq!(
            resolve_version_code(Some(83), Some(7), Some("91")),
            (83, false)
        );
    }
    ...
    fn invalid_ci_run_number_falls_back_to_package_version_code() {
        assert_eq!(
            resolve_version_code(None, Some(7), Some("not-a-number")),
            (7, false)
        );
    }
    ...
    fn entitlement_sync_is_idempotent_and_normalizes_whitespace() {
        ...
        let groups = vec!["group.example.app".to_string()];
        update_entitlements_file(&path, &groups).unwrap();
